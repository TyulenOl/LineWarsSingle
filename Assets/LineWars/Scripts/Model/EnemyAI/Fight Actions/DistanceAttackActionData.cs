...
namespace LineWars.Model
{
    public class DistanceAttackActionData : EnemyActionData
    {
        ...
        {
            if (executor is not DistanceUnit distanceUnit) return;
            ...
            while (queue.Count >= 0)
            {
                ...
                foreach (var neighborNode in currentNodeInfo.Item1.GetNeighbors())
                {
                    ...
                    if (pointsAfterMove >= 0 &&
                        (int) edge.LineType >= (int) distanceUnit.Passability
        ...
        private List<Unit> CheckDistanceAttack(Node node, DistanceUnit distanceUnit, HashSet<Unit> enemySet)
        {
    ...
    public class DistanceAttackAction : EnemyAction
    {
        ...
        {
            if (executor is not DistanceUnit distanceUnit)
            {

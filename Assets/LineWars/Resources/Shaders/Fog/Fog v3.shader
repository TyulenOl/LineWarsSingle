Shader "Custom/FogV3"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                //float offset = _Time * _NoiseSpeed;
                fixed4 tex = tex2D(_MainTex, i.uv);
                
                float maskVal = tex2D(_VisibilityMask, i.uv).a;
                float noise = tex2D(_Noise, i.uv).r;
                
                float maskNoise = clamp(maskVal - noise * pow(1.0f - maskVal, 0.001) * noise, 0, 1);
                ...
                //return tex2D(_VisibilityMask, i.uv);
            }
            ENDCG
        }
    }
